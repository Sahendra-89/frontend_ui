...
const AdminDashboard = () => {
    ...
    const [adminForm, setAdminForm] = useState({ username: '', password: '', role: 'employee' });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (newsRes.data.data) {
                ...
                setTotalArticles(newsRes.data.totalItems);
            } else {
                setNews(newsRes.data);
                setTotalArticles(newsRes.data.length);
            }
    ...
    const handleNewsSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (fileInput) fileInput.value = '';
            const res = await getNews({ page, limit: 10, search: searchTerm, category: filterCategory });
            if (res.data.data) {
                setNews(res.data.data);
                setTotalPages(res.data.totalPages);
                setTotalArticles(res.data.totalItems);
            } else {
                setNews(res.data);
    ...
    const handleAdminSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setAdminForm({ username: '', password: '', role: 'employee' });
            const res = await getAdmins();
            setAdmins(res.data);
        } catch (err: any) {
            showFeedback(err.response?.data?.msg || 'Error adding admin.', 'error');
        }
    };
                    ...
                    <div className="flex items-center space-x-6">
                        ...
                        <div className="flex items-center space-x-3 border-l pl-6">
                            ...
                            <div className="hidden md:block">
                                <p className="text-sm font-bold text-gray-800">{localStorage.getItem('username') || 'Admin User'}</p>
                                <p className="text-xs text-gray-500 capitalize">{role?.replace('admin', ' Admin') || 'Super Admin'}</p>
                            </div>
                        </div>
                    </div>
                </header>
                            ...
                            <div className="bg-[#FFE2E5] rounded-2xl p-8 mb-8 flex items-center justify-between relative overflow-hidden">
                                <div className="relative z-10 w-2/3">
                                    ...
                                    <p className="text-gray-700">
                                        You have <span className="font-bold">{totalArticles}</span> published articles and <span className="font-bold">{admins.length}</span> active team members.
                                    </p>
                                    <div className="mt-6 flex gap-4">
                                        <button onClick={() => setEditingNewsId(null)} className="bg-[#FA5A7D] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#d43d60] transition-colors shadow-lg shadow-pink-200">
                                            Write New Post
                                        </button>
                                        {role === 'superadmin' && (
                                            <button onClick={() => setActiveTab('admins')} className="bg-white text-gray-900 border border-gray-200 px-6 py-3 rounded-xl font-bold hover:bg-gray-50 transition-colors shadow-sm">
                                                Manage Team
                                            </button>
                                        )}
                                    </div>
                                ...
                                <form onSubmit={handleNewsSubmit} className="space-y-6">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="h-80 mb-12 bg-white rounded-xl overflow-hidden border border-gray-200 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-500 transition-all">
                                            <ReactQuill theme="snow" value={newsForm.content} onChange={handleContentChange} className="h-full border-none" modules={{ toolbar: [[{ 'header': [1, 2, 3, false] }], [{ 'size': [] }], ['bold', 'italic', 'underline'], [{ 'list': 'ordered' }, { 'list': 'bullet' }], ['link', 'image']] }} />
                                        </div>
                                    </div>
